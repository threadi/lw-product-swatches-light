...
class Transient {
	...
	public function display(): void {
		...
		if ( $this->has_message() ) {
			?>
			<div class="product-swatches-transient updated <?php echo esc_attr( $this->get_type() ); ?>" data-dismissible="<?php echo esc_attr( $this->get_name() ); ?>-<?php echo absint( $this->get_dismissible_days() ); ?>">
				...
				if ( $this->get_dismissible_days() > 0 ) {
					...
					$title = sprintf( __( 'Hide this message for %1$d days.', 'product-swatches-light' ), $this->get_dismissible_days() );
					?>
					<button type="button" class="notice-dismiss" title="<?php echo esc_attr( $title ); ?>"><?php echo esc_html__( 'Dismiss', 'product-swatches-light' ); ?><span class="screen-reader-text"><?php echo esc_html( $title ); ?></span></button>
					<?php
				}
				?>
			</div>
	...
	private function get_admin_transient_dismiss_cache(): string|int|false {
		$cache_key = 'lwps-dismissed-' . md5( $this->get_name() );
	...
	public function delete_dismiss(): void {
		delete_option( 'lwps-dismissed-' . md5( $this->get_name() ) );
	}
	...
	public function get_hide_on(): array {
		 ...
		 */
		return apply_filters( 'product_swatches_light_transient_hide_on', $hide_on, $this );
	}
