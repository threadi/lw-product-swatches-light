...
class Transients {
	...
	public function init(): void {
		...
		// use our own hooks.
		add_filter( 'product_swatches_light_transient_hide_on', array( $this, 'set_default_pages_where_transients_are_hidden' ) );
	...
	public function get_transients(): array {
		...
		$transients_from_db = get_option( LW_SWATCHES_TRANSIENTS_LIST, array() );
	...
	public function add_transient( Transient $transient_obj ): void {
		...
		update_option( LW_SWATCHES_TRANSIENTS_LIST, $transients_in_db );
	}
	...
	public function check_transients(): void {
		 ...
		 */
		foreach ( apply_filters( 'product_swatches_light_get_transients_for_display', $transients ) as $transient_obj ) {
	...
	public function dismiss_transient_via_ajax(): void {
		// check nonce.
		check_ajax_referer( 'product-swatches-light-dismiss-nonce', 'nonce' );
		...
		// save value.
		delete_option( 'lwps-dismissed-' . md5( $option_name ) );
		add_option( 'lwps-dismissed-' . md5( $option_name ), $dismissible_length, '', true );
