...
function lw_generate_product_swatches_button(): void {
	...
	?><a href="<?php echo esc_url( wp_nonce_url( $url, 'lws-generate' ) ); ?>" class="button button-large lw-update-swatches"><?php echo esc_html__( 'Regenerate all swatches', 'product-swatches-light' ); ?></a> (<i><?php echo esc_html__( 'takes a moment', 'product-swatches-light' ); ?></i>)
	<?php
}
 ...
 *
 * @param string     $add_to_cart_html The HTML for the cart.
 ...
 *
 * @param string     $html The HTML-code for the block.
 ...
 * @noinspection PhpUnused
 */
function lw_swatches_run_product_action(): void {
	...
	set_transient(
		'lwSwatchesMessage',
		array(
			'message' => '<strong>' . __( 'The swatches of the product have been updated.', 'product-swatches-light' ) . '</strong>',
			'state'   => 'success',
		)
	);

	// Redirect to the edit screen for the new draft page.
	wp_safe_redirect( get_edit_post_link( $product_id ) );
}
 ...
 * @param string $action The used action.
 * @param array  $post_ids List of IDs.
 * @return string
 * @noinspection PhpUnused
 * @noinspection PhpUnusedParameterInspection
 */
function lw_swatches_run_bulk_actions( string $redirect_to, string $action, array $post_ids ): string {
 ...
 *
 * @param array $attribute_types The attribute types.
 * @return array
 * @noinspection PhpUnused
 */
 ...
 *
 * @param object $attribute_taxonomy The object.
...
function lw_swatches_product_option_terms( object $attribute_taxonomy, int $i ): void {
	...
	if ( array_key_exists( $attribute_taxonomy->attribute_type, helper::get_attribute_types() ) ) {
		...
		$post_id    = absint( filter_input( INPUT_POST, 'post_id', FILTER_SANITIZE_FULL_SPECIAL_CHARS ) );
		if ( is_null( $thepostid ) && $post_id > 0 ) {
			$product_id = $post_id;
		}
